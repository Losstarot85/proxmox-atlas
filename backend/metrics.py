...
NODE_STORAGE_TOTAL = Gauge("proxmox_node_storage_total_bytes", "Total node storage capacity", ["cluster", "node", "storage"])
NODE_STORAGE_USED = Gauge("proxmox_node_storage_used_bytes", "Used node storage space", ["cluster", "node", "storage"])
NODE_STORAGE_AVAIL = Gauge("proxmox_node_storage_avail_bytes", "Available node storage space", ["cluster", "node", "storage"])
# VM/LXC metrics
VM_CPU = Gauge("proxmox_vm_cpu_usage_ratio", "VM/LXC CPU usage ratio (0.0-1.0)", ["cluster", "node", "vmid", "type", "name"])
