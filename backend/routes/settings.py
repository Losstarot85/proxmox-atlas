...
async def update_settings(data: SettingsUpdate):
    ...
    if data.polling_interval < 5:
        raise HTTPException(
            status_code=400, detail="Minimum polling interval is 5 seconds to avoid overloading Proxmox."
        )

    new_settings = {"polling_interval": data.polling_interval, "webhooks": [w.model_dump() for w in data.webhooks]}
